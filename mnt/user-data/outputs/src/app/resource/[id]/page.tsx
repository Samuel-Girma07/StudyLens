...
import { HERO_SIZES, GRID_2_SIZES, THUMBNAIL_SMALL_SIZES, getBlurPlaceholder, BLUR_PLACEHOLDER_GREY } from "@/lib/image-utils"
    ...
    <AppLayout>
      <div className="p-8 lg:p-12">
        ...
        <nav className="flex items-center gap-2 mb-8">
          <button onClick={() => router.back()} className="group flex items-center gap-2 text-slate-500 hover:text-accent-cyan transition-colors">
            <span className="material-symbols-outlined text-sm group-hover:-translate-x-1 transition-transform">arrow_back</span>
            <span className="font-mono text-[10px] uppercase tracking-widest">Back</span>
          </button>
          <span className="text-slate-700">/</span>
          <Link href="/" className="font-mono text-[10px] uppercase tracking-widest text-slate-500 hover:text-accent-cyan transition-colors">Browse</Link>
          <span className="text-slate-700">/</span>
          <span className="font-mono text-[10px] uppercase tracking-widest text-accent-cyan">{resource.title.slice(0, 20)}...</span>
        </nav>
          ...
          <div className="relative z-10 pt-8 lg:pt-16 pb-8">
            ...
            <div className="flex flex-wrap items-center gap-6 text-slate-400">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-xs uppercase tracking-widest">{resource.viewCount.toLocaleString()} views</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            {/* Hero thumbnail — priority + large sizes */}
              ...
              <div className="aspect-video relative">
                {resource.thumbnail ? (
                  <Image
                    ...
                    sizes={HERO_SIZES}
                    priority                          // above the fold — always eager
                    quality={85}                      // slightly higher quality for hero
                    placeholder="blur"
                    blurDataURL={getBlurPlaceholder(resource.type)}
                  />
                ) : (
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <button onClick={() => handleRate(1)} className={`px-6 py-4 font-mono text-xs uppercase tracking-widest border transition-all flex items-center gap-2 ${userRating === 1 ? 'bg-accent-lime text-black border-accent-lime shadow-[2px_2px_0px_#00FFFF]' : 'border-white/20 text-slate-400 hover:border-accent-lime hover:text-accent-lime'}`}>
                <span className="material-symbols-outlined text-sm">thumb_up</span>
                {resource.likeCount}
              </button>
              <button onClick={() => handleRate(-1)} className={`px-4 py-4 font-mono text-xs uppercase tracking-widest border transition-all ${userRating === -1 ? 'bg-red-500/20 text-red-400 border-red-500/50' : 'border-white/20 text-slate-400 hover:border-red-500/50 hover:text-red-400'}`}>
                <span className="material-symbols-outlined text-sm">thumb_down</span>
              </button>
              <button onClick={handleSave} className={`px-6 py-4 font-mono text-xs uppercase tracking-widest border transition-all flex items-center gap-2 ${isSaved ? 'bg-accent-cyan/20 text-accent-cyan border-accent-cyan/50' : 'border-white/20 text-slate-400 hover:border-accent-cyan hover:text-accent-cyan'}`}>
                <span className="material-symbols-outlined text-sm">{isSaved ? 'bookmark_added' : 'bookmark'}</span>
                {isSaved ? 'Saved' : 'Save'}
              </button>
              <button onClick={handleShare} className="px-4 py-4 font-mono text-xs uppercase tracking-widest border border-white/20 text-slate-400 hover:border-white/40 hover:text-white transition-all">
                    ...
                    >
                      <div className="relative aspect-video">
                        <Image
                          ...
                          quality={75}
                          placeholder="blur"
                          blurDataURL={BLUR_PLACEHOLDER_GREY}
                        />
          ...
          <div className="space-y-8">
            ...
            {/* Similar Resources — small thumbnails */}
            <div className="card-brutalist p-6">
                      ...
                      <div className={`relative w-16 h-20 shrink-0 bg-gradient-to-br ${subjectGradients[similar.subject]} flex items-center justify-center overflow-hidden`}>
                        {similar.thumbnail ? (
                          <Image
                            ...
                            alt={`Thumbnail for: ${similar.title}`}
                            fill
                            className="object-cover"
                            sizes={THUMBNAIL_SMALL_SIZES}
                            loading="lazy"
                            quality={60}              // very small size — lower quality is fine
                          />
                        ) : (
