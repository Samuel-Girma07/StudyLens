...
function BrowseContent() {
  ...
  const handleRate = async (resourceId: string, rating: 1 | -1) => {
    ...
    try {
      const isExternal = isExternalResource(currentResource)
      const url = isExternal ? '/api/resources/external/rate' : `/api/resources/${resourceId}/rate`
      const payload = isExternal ? { resource: currentResource, rating } : { rating }
  ...
  const handleSave = async (resourceId: string, currentlySaved: boolean) => {
    ...
    try {
      ...
      const isExternal = isExternalResource(currentResource)
      const url = isExternal ? '/api/resources/external/save' : `/api/resources/${resourceId}/save`
      const payload = isExternal ? (currentlySaved ? { id: resourceId } : currentResource) : undefined
                    ...
                    <div className="aspect-[16/10] overflow-hidden relative">
                      ...
                      {isExternal && resource.thumbnail ? (
                        <Image 
                          ...
                          alt={`Thumbnail graphic for resource: ${resource.title}`}
                          fill
                    ...
                    <div className="p-6 relative">
                      ...
                      <div className="flex justify-between items-center border-t border-white/5 pt-4 mt-auto">
                        <div className="flex items-center gap-2">
                          ...
                          {isExternal && (
                            <span className={`font-mono text-[10px] uppercase tracking-widest flex items-center gap-1 ml-2 ${externalBadge.text.replace('text-white', 'text-white/70')}`}>
    ...
    <Suspense fallback={
      <AppLayout>
        <div className="flex flex-col items-center justify-center min-h-[500px]">
          <span className="material-symbols-outlined text-4xl text-accent-cyan animate-spin">progress_activity</span>
          <p className="font-mono text-xs text-slate-500 uppercase tracking-widest mt-4">Loading resources...</p>
        </div>
      </AppLayout>
    }>
      <BrowseContent />
